...
xmax = 1.0;
ymin = -0.5;
y1 = -0.2375; // top of LHS
ymax = 0;
zmin = 0;
zmax = 1;

fault_thickness = 0.02;
slope = 0; // slope of middle layer in degrees

/****** MESH ***********/
nb_cells_x = 80; // nb cells in X dir along fault
...
// if mesh_type == 0
lc1 = 0.1; // mesh characteristic length outside the fault (for triangular mesh)
lc2 = 0.02; // mesh characteristic length in fault (for triangular mesh)
// if mesh_type == 1 or 2
progress_coeff = 0.98; // progression coefficient (for denser regular mesh towards fault)
// if mesh_type == 1
...
Physical Line(0) = {9};                     // top
Physical Line(1) = {1};                     // bottom
Physical Line(2) = {10,7,4};                // left
Physical Line(3) = {8,5,2};                 // right
Physical Line(4) = {3};                     // fault upper
Physical Line(5) = {6};                     // fault lower


Physical Surface(0) = {11};                 // bottom
Physical Surface(1) = {12};                 // fault
Physical Surface(2) = {13};                 // top
